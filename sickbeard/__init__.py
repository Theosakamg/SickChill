...
def initialize(consoleLogging=True):
    with INIT_LOCK:
        ...
        global BRANCH, GIT_RESET, GIT_REMOTE, GIT_REMOTE_URL, CUR_COMMIT_HASH, CUR_COMMIT_BRANCH, LOG_DIR, LOG_NR, LOG_SIZE, WEB_PORT, WEB_LOG,\
            ...
            ENABLE_HTTPS, HTTPS_CERT, HTTPS_KEY, HANDLE_REVERSE_PROXY, USE_NZBS, USE_TORRENTS, USE_DDLS, NZB_METHOD, NZB_DIR, DOWNLOAD_PROPERS, RANDOMIZE_PROVIDERS, \
        ...
        USE_DDLS = check_setting_bool(CFG, 'General', 'use_ddls', True)
        ...
        for curProvider in providers.sortedProviderList():
            ...
            if hasattr(curProvider, "storageProviderAllow"):

                curProvider.storageProviderAllow['Uptobox'] = bool(check_setting_int(CFG, curProvider.get_id().upper(),
                                                                     curProvider.get_id() + '_allow_uptobox',
                                                                     0))

                curProvider.storageProviderAllow['Uplea'] = bool(check_setting_int(CFG, curProvider.get_id().upper(),
                                                                     curProvider.get_id() + '_allow_uplea',
                                                                     0))

                curProvider.storageProviderAllow['1fichier'] = bool(check_setting_int(CFG, curProvider.get_id().upper(),
                                                                     curProvider.get_id() + '_allow_1fichier',
                                                                     0))

                curProvider.storageProviderAllow['Uploaded'] = bool(check_setting_int(CFG, curProvider.get_id().upper(),
                                                                     curProvider.get_id() + '_allow_uploaded',
                                                                     0))

                curProvider.storageProviderAllow['Rapidgator'] = bool(check_setting_int(CFG, curProvider.get_id().upper(),
                                                                     curProvider.get_id() + '_allow_rapidgator',
                                                                     0))

                curProvider.storageProviderAllow['TurboBit'] = bool(check_setting_int(CFG, curProvider.get_id().upper(),
                                                                     curProvider.get_id() + '_allow_turboBit',
                                                                     0))
...
def save_config():
    ...
    for curProvider in providers.sortedProviderList():
        ...
        if hasattr(curProvider, "storageProviderAllow"):
            new_config[curProvider.get_id().upper()][
                curProvider.get_id() + '_allow_uptobox'] = int(curProvider.storageProviderAllow['Uptobox'])

            new_config[curProvider.get_id().upper()][
                curProvider.get_id() + '_allow_uplea'] = int(curProvider.storageProviderAllow['Uplea'])

            new_config[curProvider.get_id().upper()][
                curProvider.get_id() + '_allow_1fichier'] = int(curProvider.storageProviderAllow['1fichier'])

            new_config[curProvider.get_id().upper()][
                curProvider.get_id() + '_allow_uploaded'] = int(curProvider.storageProviderAllow['Uploaded'])

            new_config[curProvider.get_id().upper()][
                curProvider.get_id() + '_allow_rapidgator'] = int(curProvider.storageProviderAllow['Rapidgator'])

            new_config[curProvider.get_id().upper()][
                curProvider.get_id() + '_allow_turboBit'] = int(curProvider.storageProviderAllow['TurboBit'])
